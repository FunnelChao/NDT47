...
def create_1d_absolute_sincos_embedding(n_pos_vec, dim, device):
    """
    usage:
        n_pos = 4
        dim = 4
        ...
        position_embedding.shape : [4,4]
    """
    assert dim % 2 == 0, "dim must be even"
    # 常数初始化一下position_embedding
    position_embedding = torch.zeros(n_pos_vec.numel(), dim, dtype=torch.float, device=device)
    # 完成指数部分的初始化
...
class NDT47(nn.Module):
    ...
    def __init__(self, input_dim=256, d_model=512, nhead=8, num_encoder_layers=6,
        ...
        self.out_token = nn.Embedding(1, d_model)
        self._reset_parameters()
    ...
    def forward(self, samples):
        ...
        pos_embedd = create_1d_absolute_sincos_embedding(n_pos_vec=torch.arange(new_seq_len, dtype=torch.float), dim=self.d_model, device=src.device)[:,None,:].repeat(1,bsz,1)
